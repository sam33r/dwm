...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->tabwin) {
    ...
    if (ev->button != Button4 && ev->button != Button5) focus(c);
    restack(selmon);
...
void configurenotify(XEvent *e) {
  ...
  if (ev->window == root) {
    ...
    if (updategeom() || dirty) {
      ...
      updatebars();
      // refreshing display of status bar. The tab bar is handled by the
      // arrange() method, which is called below
      for (m = mons; m; m = m->next) {
        resizebarwin(m);
